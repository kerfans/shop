...
{block name=content}
    ...
    <form action="" method="post" enctype="multipart/form-data">
        <div class="col-lg-11">
            <h1 class="text-center" style="color: red">实体商品详情</h1>
            <div class="form-horizontal" role="form" >
                ...
                <fieldset>
                    ...
                    <table class="table">
                        <thead>
                        <tr>
                            {foreach from=$data[2]['picture'] item="foo"}
                                <img src="http://img.yaofang.cn/{$foo}">&nbsp;&nbsp;
                            {/foreach}
                        </tr>
                        </thead>
                    </table>
            ...
            <fieldset>
                <div class="form-group">
                    <label class="col-sm-2 control-label" for="ds_host" style="color: red">货架显示名称<br>（当前设置名称）</label>
                    <div class="col-sm-4">
                        <input class="form-control" id="ds_host" name="title" type="text" value="{$data[2]['title']}" placeholder="请填入货架显示名称" required/>
                    </div>
                    <label class="col-sm-2 control-label" for="ds_name" style="color: red">商品数量<br>(当前库存)</label>
                    <div class="col-sm-4">
                        <input class="form-control" id="ds_name" name="stock_num" type="text" value="{$data[2]['stock_num']}" placeholder="请输入商品数量" required/>
                    </div>
                </div>
            </fieldset>
            <fieldset>
                <div class="form-group">
                    <label class="col-sm-2 control-label" for="ds_username" style="color: red">兑换需要积分<br>(当前积分)</label>
                    <div class="col-sm-4">
                        <input class="form-control" type="text" value="{$data[2]['cost_points']}" name="cost_points" placeholder="请输入兑换需求积分" required/>
                    </div>
                    <label class="col-sm-2 control-label" for="ds_password" style="color: red" >权重<br>(越大越靠前)</label>
                    <div class="col-sm-4">
                        <input class="form-control" type="text" name="sort" value="{$data['2']['sort']}" placeholder="请输入权重（越大越靠前显示）" required/>
                    </div>
                </div>
            </fieldset>
            <br>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        <select id="disabledSelect" name="is_sale" class="form-control">
                            <option value="1"{if $data[2]['is_sale'] == 1} selected{/if}>上架</option>
                            <option value="0"{if $data[2]['is_sale'] == 0} selected{/if}>下架</option>
                        </select>
                    </div>
                    ...
                    <div class="col-sm-4">
                        <select id="disabledSelect" name="is_hot" class="form-control">
                            <option value="0"{if $data[2]['is_hot'] == 0} selected{/if}>普通</option>
                            <option value="1"{if $data[2]['is_hot'] == 1} selected{/if}>热销</option>
                        </select>
                    </div>
                </div>
                <br><br>
                    ...
                    <div class="col-sm-4">
                        <select name="tid" class="form-control">
                            {foreach from=$types item="foo"}
                                <option value="{$foo->id}"{if $data[2]['tid'] == ($foo->id)} selected{/if}>{$foo->name}</option>
                            {/foreach}
                        </select>
                    </div>
                </div>
            ...
            <fieldset>
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        <textarea name="describe" id="" cols="50" rows="10" required>{$data[2]['describe']}</textarea>
                    </div>
                </div>
            </fieldset>
            <fieldset>
